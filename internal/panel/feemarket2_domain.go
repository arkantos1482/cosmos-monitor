...
func feemarket2DomainCardsHTML(s feemarket2.State, d model.Report) string {
	return ecoDomainsWrap(
		fm2ModuleCardHTML(s, d),
		fm2ConsensusCardHTML(s, d),
		fm2NodePolicyCardHTML(s),
	)
}

func fm2ModuleCardHTML(s feemarket2.State, d model.Report) string {
	var b strings.Builder
	ecoDomainCardOpen(&b, "eco-domain--feemarket2", "x/feemarket params", "on-chain module store")
...
func fm2ConsensusCardHTML(s feemarket2.State, d model.Report) string {
	var b strings.Builder
	ecoDomainCardOpen(&b, "eco-domain--feemarket2-consensus", "Consensus limits", "CometBFT block caps")
	limit := "unlimited"
	if s.GasLimit > 0 {
...
func fm2NodePolicyCardHTML(s feemarket2.State) string {
	var b strings.Builder
	ecoDomainCardOpen(&b, "eco-domain--feemarket2-node", "Node policy", "local app.toml / mempool")
...
func fm2ProjectedFee(s feemarket2.State) string {
	if !s.HasProjection {
		return "—"
	}
